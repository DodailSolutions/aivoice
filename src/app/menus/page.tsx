...
export default function MenusPage() {
  ...
  const [active, setActive] = useState<any>(CUISINES[0]);
  ...
  useEffect(() => {
    getContent()
      .then((data) => {
        setContent(data);
        if (data?.menus?.cuisines && data.menus.cuisines.length > 0) {
          setActive(data.menus.cuisines[0]);
        }
      })
      .catch(console.error);
  }, []);

  const cuisines = content?.menus?.cuisines || CUISINES;
